...
export class MemoryStore implements Store {
  ...
  async billImpression(input: BillImpressionInput): Promise<void> {
    ...
    if (campaign && input.chargeCents > 0) {
      campaign.spentTodayCents += input.chargeCents;
    ...
    bucket.grossCents += input.chargeCents;
    bucket.devShareCents = devShareCents(bucket.grossCents, input.revShareBps);
  }
